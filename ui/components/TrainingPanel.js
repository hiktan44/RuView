...
export default class TrainingPanel {
  ...
  _renderHeader() {
    ...
    h.appendChild(this._el('span', 'tp-title', 'Eğitim'));
    const ts = this.state.trainingStatus;
    let cls = 'tp-badge tp-badge-idle', txt = 'Boşta';
    if (ts && ts.active) { cls = 'tp-badge tp-badge-active'; txt = 'Eğitim Yapılıyor'; }
    else if (ts && !ts.active && this.progressData.losses.length > 0) { cls = 'tp-badge tp-badge-done'; txt = 'Tamamlandı'; }
  ...
  _renderRecordings() {
    ...
    s.appendChild(this._el('div', 'tp-section-title', 'CSI Kayıtları'));
    if (this.state.recordings.length === 0 && !this.state.loading) {
      s.appendChild(this._el('div', 'tp-empty', 'Model eğitmek için CSI verisi kaydedin'));
    } else {
      this.state.recordings.forEach(rec => {
        ...
        const del = this._btn('Sil', 'tp-btn tp-btn-muted', () => this._delRec(rec.id));
    ...
    if (this.state.isRecording) {
      const b = this._btn('Kaydı Durdur', 'tp-btn tp-btn-danger', () => this._stopRec());
      b.disabled = this.state.loading; acts.appendChild(b);
    } else {
      const b = this._btn('Kayıt Başlat', 'tp-btn tp-btn-rec', () => this._startRec());
      b.disabled = this.state.loading; acts.appendChild(b);
    }
    s.appendChild(acts);
    return s;
  ...
  _renderConfig() {
    ...
    hdr.appendChild(this._el('span', 'tp-section-title', 'Eğitim Yapılandırması'));
    hdr.appendChild(this._btn(this.state.configOpen ? 'Daralt' : 'Genişlet', 'tp-btn tp-btn-muted',
    ...
    if (this.state.recordings.length > 0) {
      form.appendChild(this._el('label', 'tp-label', 'Veri Kümeleri'));
    ...
    form.appendChild(ir('Epok Sayısı', 'number', this.config.epochs, v => { this.config.epochs = parseInt(v) || 100; }));
    form.appendChild(ir('Yığın Boyutu', 'number', this.config.batch_size, v => { this.config.batch_size = parseInt(v) || 32; }));
    form.appendChild(ir('Öğrenme Oranı', 'text', this.config.learning_rate, v => { this.config.learning_rate = parseFloat(v) || 3e-4; }));
    form.appendChild(ir('Erken Durdurma Sabrı', 'number', this.config.patience, v => { this.config.patience = parseInt(v) || 15; }));
    form.appendChild(ir('Temel Model (isteğe bağlı)', 'text', this.config.base_model, v => { this.config.base_model = v; }));
    form.appendChild(ir('LoRA Profili (isteğe bağlı)', 'text', this.config.lora_profile_name, v => { this.config.lora_profile_name = v; }));
    s.appendChild(form);

    const acts = this._el('div', 'tp-train-actions');
    const btns = [
      this._btn('Eğitimi Başlat', 'tp-btn tp-btn-success', () => this._launchTraining('startTraining', { patience: this.config.patience, base_model: this.config.base_model || undefined })),
      this._btn('Ön Eğitim', 'tp-btn tp-btn-secondary', () => this._launchTraining('startPretraining')),
  ...
  _renderProgress() {
    const ts = this.state.trainingStatus || {};
    const s = this._el('div', 'tp-section');
    s.appendChild(this._el('div', 'tp-section-title', 'Eğitim İlerlemesi'));
    ...
    s.appendChild(this._el('div', 'tp-progress-label', `Epok ${ts.epoch ?? 0} / ${ts.total_epochs ?? '?'}  (${pct}%)`));
    ...
    g.appendChild(mc('Kayıp', ts.train_loss != null ? ts.train_loss.toFixed(4) : '--'));
    ...
    g.appendChild(mc('En İyi PCK', ts.best_pck != null ? (ts.best_pck * 100).toFixed(1) + '% (e' + (ts.best_epoch ?? '?') + ')' : '--'));
    g.appendChild(mc('Sabır', ts.patience_remaining != null ? String(ts.patience_remaining) : '--'));
    g.appendChild(mc('Tahmini Süre', ts.eta_secs != null ? this._fmtEta(ts.eta_secs) : '--'));
    g.appendChild(mc('Aşama', ts.phase || '--'));
    s.appendChild(g);

    const stop = this._btn('Eğitimi Durdur', 'tp-btn tp-btn-danger', () => this._stopTraining());
  ...
  _renderComplete() {
    const ts = this.state.trainingStatus || {};
    const s = this._el('div', 'tp-section');
    s.appendChild(this._el('div', 'tp-section-title', 'Eğitim Tamamlandı'));
    const g = this._el('div', 'tp-metrics-grid');
    const mc = (l, v) => { const c = this._el('div', 'tp-metric-cell'); c.appendChild(this._el('div', 'tp-metric-label', l)); c.appendChild(this._el('div', 'tp-metric-value', v)); return c; };
    ...
    s.appendChild(g);
    const acts = this._el('div', 'tp-train-actions');
    acts.appendChild(this._btn('Yeni Eğitim', 'tp-btn tp-btn-secondary', () => {
  ...
  _drawCharts() {
    this._drawChart('tp-loss-chart', this.progressData.losses, { color: '#ff6b6b', label: 'Kayıp', yMin: 0, yMax: null });
